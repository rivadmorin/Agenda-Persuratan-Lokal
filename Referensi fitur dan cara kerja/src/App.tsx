        ...
        <AnimatePresence>
          {activeTab === 'agenda' && showPreviewPane && selectedMail && (
            ...
            >
              ...
              {/* Tab Selector for PDF / Markdown View - Refactored to Google Material M3 Tabs */}
              <div className="bg-white dark:bg-slate-900 border-b border-slate-150 dark:border-slate-800/85">
                <md-tabs
                  activeTabIndex={previewMode === 'details' ? 0 : previewMode === 'pdf' ? 1 : 2}
                  onchange={(e: any) => {
                    const idx = e.target.activeTabIndex;
                    if (idx === 0) setPreviewMode('details');
                    else if (idx === 1) setPreviewMode('pdf');
                    else if (idx === 2) setPreviewMode('markdown');
                  }}
                  style={{
                    '--md-tabs-container-color': 'transparent',
                    '--md-tabs-active-indicator-color': 'var(--theme-base, #2563eb)',
                    '--md-primary-tab-active-label-text-color': 'var(--theme-base, #2563eb)',
                    '--md-primary-tab-active-icon-color': 'var(--theme-base, #2563eb)',
                    '--md-primary-tab-label-text-size': '12px',
                    '--md-primary-tab-label-text-weight': '700',
                  } as React.CSSProperties}
                  className="w-full"
                >
                  <md-primary-tab active={previewMode === 'details' ? true : undefined}>
                    <Eye slot="icon" className="w-4 h-4 inline-block" />
                    <span>Rincian</span>
                  </md-primary-tab>
                  <md-primary-tab active={previewMode === 'pdf' ? true : undefined}>
                    <FileText slot="icon" className="w-4 h-4 inline-block" />
                    <span>Berkas PDF</span>
                  </md-primary-tab>
                  <md-primary-tab active={previewMode === 'markdown' ? true : undefined}>
                    <BookOpen slot="icon" className="w-4 h-4 inline-block" />
                    <span>Markdown</span>
                  </md-primary-tab>
                </md-tabs>
              </div>
