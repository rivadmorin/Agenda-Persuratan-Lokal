  ...
  return (
    <div className="flex-1 flex h-screen select-none relative overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden p-6 gap-5">
        {/* Top Control Bar - Styled like an M3 Tonal Surface */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 p-4 border border-slate-200/60 dark:border-slate-800 rounded-3xl shadow-sm transition-all duration-300">
          {/* Search bar - M3 Outlined Search Pill */}
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-11 pr-16 py-2.5 rounded-full bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800/80 text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-slate-900 focus:ring-1 focus:ring-blue-500 transition-all text-sm font-medium"
              placeholder="Cari semua data surat..."
            />
            {globalSearch && (
              <span className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[9px] bg-blue-500/10 text-blue-600 dark:text-blue-400 font-bold px-2 py-0.5 rounded-full border border-blue-500/20 select-none uppercase tracking-wider">
                Fuzzy
              </span>
            )}
          </div>

          {/* Action buttons - Beautiful M3 Rounded Capsules & FABs */}
          <div className="flex items-center gap-2.5 self-end sm:self-auto flex-wrap">
            <button
              onClick={onRefresh}
              className="p-2.5 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-750 border border-slate-200/60 dark:border-slate-850 rounded-full text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white transition-all active:scale-95 cursor-pointer shadow-xs"
              title="Segarkan Data"
            >
            ...
            {currentUser.role === 'admin' && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 border border-emerald-100/60 dark:border-emerald-950/20 bg-emerald-50/70 dark:bg-emerald-950/15 hover:bg-emerald-100 dark:hover:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 font-bold text-xs uppercase tracking-wider rounded-full transition-all active:scale-95 cursor-pointer shadow-xs"
              >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 border border-blue-100/60 dark:border-blue-950/20 bg-blue-50/70 dark:bg-blue-950/15 hover:bg-blue-100 dark:hover:bg-blue-950/30 text-blue-700 dark:text-blue-400 font-bold text-xs uppercase tracking-wider rounded-full transition-all active:scale-95 cursor-pointer shadow-xs"
            >
              <FileDown className="w-4 h-4" />
              <span>Ekspor Excel</span>
            </button>

            {/* M3 Extended FAB Style for Add Mail */}
            <button
              onClick={onAddMail}
              className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs uppercase tracking-wider rounded-full shadow-sm hover:shadow transition-all active:scale-95 cursor-pointer"
            >
        ...
        {/* Spreadsheet-like Data Table Card - Refactored as an Elegant M3 Container */}
        <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl shadow-sm flex flex-col overflow-hidden relative transition-colors duration-200">
          <div className="flex-1 overflow-auto">
            <table className="w-full border-collapse text-left text-sm table-fixed" style={{ minWidth: `${(showNoColumn ? 496 : 448) + sortedColumns.length * 224}px` }}>
              <thead className="bg-slate-50 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">
                {/* Header Labels row */}
                <tr className="divide-x divide-slate-200/60 dark:divide-slate-800/50">
                  <th className="w-12 px-3 text-center bg-slate-50/50 dark:bg-slate-800/20">
                    <input
                      type="checkbox"
                  ...
                  {sortedColumns.map((col) => (
                    <th
                      ...
                      className="w-56 px-4 py-3.5 cursor-pointer hover:bg-slate-100/50 dark:hover:bg-slate-800/60 transition-all group text-left"
                    >
                ...
                <tr className="bg-slate-100/20 dark:bg-slate-950/20 border-b border-slate-200 dark:border-slate-800 divide-x divide-slate-200/60 dark:divide-slate-800/50">
                  <th className="bg-slate-100/20 dark:bg-slate-950/10"></th>
                  {showNoColumn && <th className="bg-slate-100/20 dark:bg-slate-950/10"></th>}
 
                  {sortedColumns.map((col) => (
                    <th key={col.key} className="w-56 px-2 py-1.5 bg-slate-100/20 dark:bg-slate-950/10">
                      <input
                        type={col.type === 'number' ? 'number' : col.type === 'date' ? 'date' : 'text'}
                        value={columnFilters[col.key] || ''}
                        onChange={(e) => handleColumnFilterChange(col.key, e.target.value)}
                        className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs p-1 px-2.5 focus:outline-none focus:border-blue-500 dark:focus:border-blue-400 font-medium text-slate-800 dark:text-slate-200"
                        placeholder="Filter..."
                      />
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-200/60 dark:divide-slate-800/60 select-none">
                {(loading || fetching) ? (
                  <tr>
                  ...
                  paginatedMails.map((mail, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`divide-x divide-slate-200/60 dark:divide-slate-800/50 hover:bg-slate-50/40 dark:hover:bg-slate-800/10 transition-colors cursor-pointer select-none group/row ${
                          isSelectedForPreview ? 'bg-blue-50/30 dark:bg-blue-950/25 hover:bg-blue-50/30 dark:hover:bg-blue-950/25' : isChecked ? 'bg-slate-50/80 dark:bg-slate-800/35' : 'bg-white dark:bg-slate-900'
                        }`}
                      >
                        <td 
                        ...
                        {/* File Attachment status - Beautiful M3 Pill/Chip style */}
                        <td className="w-28 px-4 text-center" onClick={(e) => e.stopPropagation()}>
                          {mail.pdfPath ? (
                            <span
                              ...
                              className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-150 dark:border-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[11px] font-bold cursor-pointer hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-all duration-200 active:scale-95"
                            ...
                            <div className="relative inline-flex">
                              <label className="flex items-center gap-1 px-3 py-1 border border-dashed border-slate-300 dark:border-slate-700 hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-950/25 text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 text-[11px] font-bold rounded-full cursor-pointer transition-all duration-200">
                        ...
                        {/* Edit, Download & Delete actions - Styled elegantly */}
                        <td className="w-32 px-4 text-center" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-center gap-1.5">
                            <button
                              onClick={() => onEditMail(mail)}
                              className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 rounded-full transition-all cursor-pointer"
                              title="Edit Surat"
                            >
                            ...
                            {mail.pdfPath ? (
                              <button
                                ...
                                className="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-350 rounded-full transition-all cursor-pointer"
                              ...
                              <button
                                disabled
                                className="p-2 text-slate-300 dark:text-slate-700 rounded-full cursor-not-allowed"
                                title="Tidak ada lampiran PDF untuk diunduh"
                              >
                                <FileDown className="w-3.5 h-3.5 opacity-30" />
                              </button>
                            )}
                            <button
                              onClick={() => setDeleteId(mail.id)}
                              className="p-2 hover:bg-rose-50 dark:hover:bg-rose-950/20 text-rose-500 hover:text-rose-650 dark:text-slate-400 dark:hover:text-rose-400 rounded-full transition-all cursor-pointer"
                              title="Hapus Surat"
                            >
          ...
          {/* Table Footer (Pagination controls) - Styled with Circular Pills */}
          <div className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200/60 dark:border-slate-800 px-6 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4 text-xs text-slate-500 dark:text-slate-400 select-none transition-colors duration-200">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-full py-1 px-3 focus:outline-none focus:border-blue-500 font-semibold text-slate-700 dark:text-slate-300 cursor-pointer"
                >
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <button
                ...
                className="px-4 py-2 rounded-full border border-slate-200 dark:border-slate-800 hover:bg-white dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold disabled:opacity-40 select-none transition-all cursor-pointer h-9 flex items-center"
              >
                Sebelumnya
              </button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((pg) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-full border font-bold select-none transition-all cursor-pointer flex items-center justify-center ${
                    pg === currentPage
